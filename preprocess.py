...
def to_df(data):
    ...
    # Extraire les données 'properties'
    properties_data = [feature['properties'] for feature in data['features']]

    # Normaliser les données et les charger dans un DataFrame Pandas
...
def update_titles(my_df):
    ...
    my_df = my_df.replace({'TYPE_SITE_INTERVENTION': TITLES})
...
def get_neighborhoods(montreal_data):
    ...
    # Extraire les données 'properties'
    properties_data = [feature['properties'] for feature in montreal_data['features']]

    # Normaliser les données et les charger dans un DataFrame Pandas
    df = pd.json_normalize(properties_data)

    # Obtenir le nom des arrondissements
    neighborhoods_names = df['NOM'].unique() 
    
    return neighborhoods_names
